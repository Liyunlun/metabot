...
| `/report-bug <desc>` | 报告 Bug — 引导填写信息并创建 GitHub Issue |
| `/request-feature <idea>` | 功能请求 — 完善想法并创建 GitHub Issue |
| `/fix-issue <number\|list>` | 认领并修复 GitHub Issue — 自动分支、提交、创建 PR |
...
MetaBot 内置了 Claude Code skill 来简化贡献流程。可在终端或飞书/Telegram 聊天中直接使用。

| 命令 | 说明 |
|------|------|
| `/report-bug` | 引导你描述 Bug，自动生成 GitHub Issue（含环境信息），确认后创建。 |
| `/request-feature` | 帮你完善功能想法，检查现有功能，创建结构化的功能请求 Issue。 |
| `/fix-issue [number]` | 认领 Issue，创建分支，实现修复，运行 CI，提交 PR — 全程引导。 |
...
**飞书/Telegram 使用：** 在聊天中发送相同命令（如 `/report-bug ...`）。Bot 转发给 Claude Code 执行 skill 并返回交互式卡片。
